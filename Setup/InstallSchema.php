...
class InstallSchema implements InstallSchemaInterface
{
     ...
     */
     public function createTableQuickOrder(SchemaSetupInterface $setup, ModuleContextInterface $context)
     {
            ...
            $installer->startSetup();
             /** create table 'my_quick_order_table' */
             $table = $installer->getConnection()->newTable($installer->getTable(QuickOrderInterface::TABLE_NAME)
             )->addColumn(
                 QuickOrderInterface::ID_FIELD,
                 Table::TYPE_INTEGER,
                 null,
                 ['identity' => true, 'nullable' => false, 'primary' => true, 'unsigned' => true], 'QuickOrder ID'
             )->addColumn(
     ...
     */
     public function createTableStatus(SchemaSetupInterface $setup, ModuleContextInterface $context)
     {
         $installer = $setup;
         $installer->startSetup();
         $table = $installer->getConnection()->newTable($installer->getTable(StatusInterface::TABLE_NAME)
             )->addColumn(
